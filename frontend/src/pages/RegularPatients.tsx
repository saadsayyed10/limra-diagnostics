...
const RegularPatients = () => {
  ...
  const handleFetchAllPatients = async () => {
    try {
      ...
      const res = await fetchAllPatientsAPI(token!, "REGULAR");
      setRegularData(res.data.patient);
    } catch (error: any) {
    ...
    <div className="w-full space-y-6 lg:p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b pb-5">
        ...
        <div className="flex items-center gap-2 bg-zinc-100 px-3 py-1.5 rounded-lg text-sm font-medium text-zinc-700 w-fit self-start sm:self-center">
          ...
          <span>Total Records: {regularData.length}</span>
        </div>
      </div>
                  ...
                  >
                    ...
                    <TableCell className="py-4 px-4 text-zinc-600 max-w-60">
                      <div className="flex items-start gap-1.5">
                        ...
                        <span
                          className="block"
                          title={`${pat.address.localAddress}, ${pat.address.city} - ${pat.address.pincode}, ${pat.address.state}`}
                        >
                          {pat.address.localAddress}
                        </span>
                      </div>
                    </TableCell>
